...
public class ShootingAction extends Action {
	...
	private AnyUnit GetShotEnemy() throws Exception {
		if(currentUnit instanceof Friendly) {
			for(AnyUnit enemy : enemyUnits) {
				if (((FriendlyUnit) currentUnit.unit).checkShotAgainstEnemy((EnemyUnit) enemy.unit) == ShotResult.CAN_HIT_ENEMY) {
					return enemy;
				}
			}
		}
			...
			for(AnyUnit enemy : enemyUnits) {
				if (((FriendlyUnit) enemy.unit).checkShotAgainstEnemy((EnemyUnit) currentUnit.unit) == ShotResult.CAN_HIT_ENEMY) {
					return enemy;
				}
			}
		}
